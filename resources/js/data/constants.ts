import type { TierType, StatusConfig, TierConfig, BookingStatus } from './types';
...
export const tierOrder: TierType[] = ['headliner', 'sub_headliner', 'mid_tier', 'undercard'];
...
export const scheduleStages = ['Main Stage', 'Second Stage', 'Tent Stage', 'DJ Booth'];

// Score color ranges (for ScoreBadge component)
export function getScoreColor(score: number): { bg: string; text: string } {
  if (score >= 85) return { bg: 'hsl(var(--score-high-bg))', text: 'hsl(var(--score-high))' };
  if (score >= 70) return { bg: 'hsl(var(--score-medium-bg))', text: 'hsl(var(--score-medium))' };
  if (score >= 55) return { bg: 'hsl(var(--score-low-bg))', text: 'hsl(var(--score-low))' };
  return { bg: 'hsl(var(--score-critical-bg))', text: 'hsl(var(--score-critical))' };
}
