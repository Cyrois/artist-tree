...
Http::fake([
    'api.spotify.com/*' => Http::response(['access_token' => 'fake-token']),
    'www.googleapis.com/youtube/*' => Http::response(['items' => []]),
]);
```
...
2. Search documentation using `search-docs` MCP tool
3. Run existing tests: `php artisan test --filter=relevant_test`

### After Writing Code
1. Write/update tests for new functionality
2. Run tests: `php artisan test --filter=new_test`
3. Format code: `vendor/bin/pint`
4. Check for N+1 queries using Laravel Debugbar (if installed)

### Git Commit Messages
- Use conventional commits: `feat:`, `fix:`, `refactor:`, `test:`
- Example: `feat: add artist scoring algorithm with Spotify integration`
